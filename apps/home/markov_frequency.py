...
class MarkovChain(object):
    ...
    def _normalize_transitions(self):
        """
        Normalize the transition matrix to Bigram-DCT form.
        Each row is normalized by dividing by the count of non-zero elements in that row.
        """
        dense_matrix = self.transition_matrix.toarray()  # Convert to dense matrix
        # Count non-zero elements for each row
        non_zero_counts = np.diff(self.transition_matrix.tocsr().indptr)  # Non-zero counts per row

        for row_idx, count in enumerate(non_zero_counts):
            if count > 0:  # Avoid division by zero
                dense_matrix[row_idx, :] /= count

        self.transition_matrix = sparse.dok_matrix(dense_matrix)  # Convert back to sparse
    ...
    def transition_df(self):
        """
        將轉移矩陣轉換為 Pandas DataFrame
        """
        # 將稀疏矩陣轉換為密集矩陣（可能會消耗更多記憶體）
        dense_matrix = self.transition_matrix.toarray()

        # 建立 DataFrame
        df = pd.DataFrame(
            dense_matrix,
            index=sorted(self.possible_states),
            columns=sorted(self.possible_states)
        )
